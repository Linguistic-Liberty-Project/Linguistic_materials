...
def load_data(path, path2):
    ...
    reader2 = csv.reader(open(path2, "rU"), delimiter=',')
    path = path + '.updated'
...
def train():
    traning_parameters = {'input': 'fasttext.train', 'epoch': 10000, 'lr': 1.0, 'wordNgrams': 1, 'verbose': 2,
                          'minCount': 1, 'loss': "ns",
...
def transform(input_file, input_file2, output_file):
    # load data
...
if __name__ == "__main__":
    transform("ukrainian_comments_annotated.csv","fasttext.train")
